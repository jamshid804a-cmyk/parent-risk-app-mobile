...
export default function TestingScreen() {
  ...
  function confirmDelete(test: TestRecord) {
    Alert.alert(
      "Delete Test",
      `Remove "${test.subject}" (${test.testType}) permanently?`,
      [
  ...
  async function deleteTest(test: TestRecord) {
    try {
      const res = await fetch(`${BASE_URL}/api/tests?id=${test.id}`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error("Delete failed");
      setTests((prev) => prev.filter((t) => t.id !== test.id));
    } catch (e: any) {
      Alert.alert("Error", e.message || "Could not delete test");
    }
  }
  ...
  tests.forEach((t) => {
    const key = TYPES.includes(t.testType as any) ? t.testType : "Monthly";
  ...
  if (error) {
    return (
      <View style={styles.center}>
        ...
        <TouchableOpacity onPress={fetchTests} style={styles.retryBtn}>
          ...
          {tests.length === 0 ? (
            <View style={[styles.emptyCard, { marginTop: -34 }]}>
              <View style={styles.emptyIcon}>
                <Ionicons name="flask-outline" size={30} color="#4338ca" />
              </View>
